  ...
  calculatePosition({ symbol = 'XAU/USD', strategy = null, equity, entryPrice, stopLossPrice, takeProfitPrice, currentSpread = 0, riskAmount = null, riskPercent = null }) {
    ...
    if (!effectiveRiskPercent && strategy && this.config.risk?.tieredRisk?.enabled) {
      ...
      if (sUpper.includes(rank1Key) || sUpper.includes(rank1Key.replace('ENGINE_', ''))) {
        ...
        effectiveRiskPercent = tiered.rank2RiskPercent || 5.0;
    ...
    // Giới hạn an toàn (min 0.01, max 2.00 cho Crypto/Vàng, max 5.00 cho Forex, max 20.00 cho US500)
    const configuredMaxLot = this.config.risk.maxLotSize || 2.00;
    const isForexPair = s.includes('GBP') || s.includes('EUR') || s.includes('JPY');
    const isIndex = s.includes('US500') || s.includes('SPX');
    const maxLot = isIndex 
      ? Math.max(20.00, configuredMaxLot) 
      : (isForexPair ? Math.max(5.00, configuredMaxLot) : configuredMaxLot);
